...
string ispisiKriterij(Kriteriji kriterij) {
    switch (kriterij)
    {
    case CISTOCA:
        return "CISTOCA";
...
bool ValidirajBrojPasosa(string pasos) {
    return regex_match(pasos, regex("[A-Z]{1,2}[0-9]{3,4}[- ]?[0-9]{2,4}"));
}
...
public:
    ...
    Kolekcija InsertAt(int lokacija, T1 el1, T2 el2) {
        ...
        for (int j = 0; j < *_trenutno; j++) {
            nova.AddElement(_elementi1[j], _elementi2[j]);
        }
...
public:
    ...
    friend ostream& operator<< (ostream& COUT, const Komentar& obj) {
        cout << "Komentar rezervacije:" << endl;
...
public:
   ...
   */
    float ProsjecnaOcjena()const {
        ...
        if (_komentar.GetOcjeneKriterija()->getTrenutno() == 0) return 0;
    ...
    bool AddGost(Gost gost) {
        for (int i = 0; i < _gosti.size(); i++) {
            if (gost.GetBrojPasosa() == _gosti[i].GetBrojPasosa() || gost.GetEmail() == _gosti[i].GetEmail())
                return false;
        }

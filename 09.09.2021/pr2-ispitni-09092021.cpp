...
const char* crt = "\n-------------------------------------------\n";
enum Pojas { BIJELI = 0, ZUTI, NARANDZASTI, ZELENI, PLAVI, SMEDJI, CRNI };
	...
	*/
	bool AddOcjena(int ocjena, Datum datum) {
		for (int i = 0; i < _ocjene->getTrenutno(); i++) {
			...
			//linija ispod je koristena za testiranje
			//cout << datum.toDays() - _ocjene->getElement2(_ocjene->getTrenutno()-1)->toDays() << endl;

			if (datum.toDays() - _ocjene->getElement2(_ocjene->getTrenutno()-1)->toDays() < 3)
				return false;
		}
...
public:
	...
	Dosadasnji uspjeh (prosjek ocjena) na pojasu Y iznosi F, 
	a ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi Z.
	Pozdrav.
	KARATE Team.
	...
	*/
	void SaljiMail(Tehnika tehnika, Pojas pojas, float prosjek) {
		...
		cout << "Postovani " << GetImePrezime() << ", evidentirana vam je tehnika " << endl;
		cout << tehnika.GetNaziv() << " za pojas " << ispisiPojas(pojas) << "." << endl;
		cout << "Dosadasnji uspjeh na pojasu " << ispisiPojas(pojas) << " iznosi " << prosjek << endl;
		cout << ", a ukupni uspjeh na svim pojasevima iznosi " << ProsjekPolaznika() << endl;
		cout << "Pozdrav.\nKARATE Team." << endl;
		m.unlock();
	}
